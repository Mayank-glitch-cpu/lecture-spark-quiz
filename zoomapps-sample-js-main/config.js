...
// Check that we have all our config dependencies - but only in production
let hasMissing = !config;
const isDevelopment = process.env.NODE_ENV === 'development';

if (!isDevelopment) {
    // Only enforce credentials in production environment
...
export const zoomApp = {
    ...
    clientId: config.ZM_CLIENT_ID || 'dev-client-id',
    clientSecret: config.ZM_CLIENT_SECRET || 'dev-client-secret',
    redirectUrl: config.ZM_REDIRECT_URL || 'https://localhost:3000/auth',
    sessionSecret: config.SESSION_SECRET || 'dev-session-secret',
};
